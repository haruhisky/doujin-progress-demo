...
- **Electronアプリ化**: electron-main.js + demo.html（単一HTML）+ localStorage永続化
- **マルチプロジェクト対応**: 横タブ切替、設定画面でプロジェクト管理
- **デイリー/イベントタスクをグローバル化**: プロジェクトと独立管理
- **カレンダー強化**: プロジェクト期間マーカー、イベント直接登録
- **シール削除をタップ方式に変更**（スワイプ→タップ）
- **アイコン設定**: dog_icon.png → icon.ico（犬アイコン）
- **ビルド・リリース**: electron-builder でポータブルexe（67MB）、GitHub Releases v1.0.0
- **デモページ統合**: わんたっぷデモ + Work MateデモをiframeでCSS分離して同ページに配置
  - WorkMateデモはpostMessageで状態切替ボタンを外部制御

### Phase 2 デプロイ (2026-03-02)
- **Flutter Web版**: GitHub Pages を旧HTMLデモからFlutter Webに置換
  - `flutter build web --base-href /doujin-progress-demo/`
  - ビルド成果物をリポジトリルートに配置
- **Flutter Windows版**: リリースビルド → zip化 → GitHub Releases v2.0.0
  - 日本語パス問題を回避するため一時ディレクトリ（ASCII-only path）でビルド
- **旧Electronファイル削除**: demo.html, electron-main.js, package.json, src/, config/ 等
- **pubspec.yaml**: version 2.0.0+1
- **manifest.json**: アプリ名「同人わんたっぷ」、テーマカラー #D4A0B9
- **GitHub Releases**: https://github.com/haruhisky/doujin-progress-demo/releases/tag/v2.0.0
- **GitHub Pages**: https://haruhisky.github.io/doujin-progress-demo/
